...
from langchain.chains.query_constructor.base import (
    StructuredQueryOutputParser, AttributeInfo, get_query_constructor_prompt
)
...
def create_chain(vectorStore):
    ...
    # Define metadata field information for the query constructor
    metadata_field_info = [
        AttributeInfo(
            name="subject",
            description=(
                'The subject relevant to the document. One of "Programming", "Electronics", '
                '"Embedded Systems", "3D Design", "Manufacturing", "Non-Technical", "Sports", '
                '"Business and Finance" or "Other".'
            ),
            type="string",
        ),
    ]
    ...
    # Construct the prompt for the query constructor
    ...
    # Set up the output parser and the query constructor
    output_parser = StructuredQueryOutputParser.from_components()
    query_constructor = prompt | model | output_parser

    # Create a self-query retriever with filtering capabilities
